    ...
    if title != "George Mason Federated Login Service":
        ...
        if title == "Information Release":
            ...
            for course in all_course_cards:
                ...
                div = Bot_Chrome.find_element(By.CSS_SELECTOR, '#block\:\:3-dueView\:\:3-dueView_3 > div')
                
                p = div.find_elements(By.TAG_NAME, 'p')
                for text in p:
                    print(text.get_attribute('id'))
                    print(text.get_attribute('innerText'))
                    print(text.tag_name)
                    print(text.id)

                    assignment = text.get_attribute('innerText')
                    if assignment is None:
                        assignment = 'no due assignments'
                    todos.append(assignment)
            ...
            student_assigments = ''
            for t in todos:
                student_assigments += t
                print(t)
            client.send_sms(UserNum, "Todos: " + student_assigments)
            time.sleep(45)
